...
/**
 * Find nearby events for contact locations
 * This is the main function that orchestrates event detection
 */
export async function findNearbyEvents(contacts = [], options = {}) {
    try {
        const headers = await getAuthHeaders();
        
        // Prepare locations from contacts
        const locations = contacts
            .filter(contact => contact.location?.latitude && contact.location?.longitude)
            .map(contact => ({
        ...
        if (locations.length === 0) {
            return {
                success: true,
                events: [],
                message: 'No contacts with location data found'
            };
        }

        console.log('🔍 Finding nearby events for', locations.length, 'contact locations');
        ...
        console.log('✅ Nearby events found:', {
            ...
            highConfidenceEvents: result.analytics?.highConfidenceEvents || 0,
            locationsProcessed: result.analytics?.locationsProcessed || 0
        });
...
/**
 * Auto-generate groups with enhanced event detection
 * This replaces the basic generateAutoGroups function
 */
export async function generateAutoGroupsWithEvents(options = {}) {
    try {
        ...
        const enhancedOptions = {
            ...
            enhancedEventDetection: options.enhancedEventDetection !== false,
            minGroupSize: options.minGroupSize || 2,
            maxGroups: options.maxGroups || 50,
            eventDetectionRadius: options.eventDetectionRadius || 1000,
            ...options
        };

        console.log('🤖 Generating auto groups with enhanced event detection:', enhancedOptions);
        ...
        console.log('✅ Enhanced auto-groups generated:', {
            groupsCreated: result.groupsCreated,
            eventBasedGroups: result.analytics?.eventBasedGroups || 0,
            companyGroups: result.analytics?.companyGroups || 0,
            locationGroups: result.analytics?.locationGroups || 0
        });

        return result;
    } catch (error) {
        console.error('Error generating auto groups with events:', error);
        throw error;
    }
}

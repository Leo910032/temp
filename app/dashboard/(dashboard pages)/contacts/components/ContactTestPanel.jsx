// components/ContactTestPanel.jsx - API-based test panel for contact generation
"use client"
import { useState, useEffect } from 'react';
import { toast } from 'react-hot-toast';
import { useAuth } from '@/contexts/AuthContext'; // 👈 1. IMPORT YOUR AUTH CONTEXT
...
export default function ContactTestPanel({ onContactsGenerated, className = "" }) {
  ...
  const loadGenerationInfo = async () => {
        // Add a guard clause to prevent running if currentUser is not ready
        if (!currentUser) return;

        try {
            setIsLoadingInfo(true);
            const response = await fetch(`/api/admin/generate-contacts?userId=${currentUser.uid}`);
    ...
    // Re-run loadGenerationInfo only when auth is ready
    useEffect(() => {
    ...
    // ✅ STEP 2: FIX THE EVENT HANDLER LOGIC
    const handleQuickGenerate = async (scenario, customCount = null) => {
        // Guard clauses first
        if (authLoading || !currentUser) {
            toast.error("Authentication not ready. Please wait a moment.");
            return;
        }

        setIsGenerating(true);
        const loadingToast = toast.loading(`Generating ${scenario} contacts...`);

        try {
            // Get the token *inside* the try block
            const token = await currentUser.getIdToken();

            // Check the token *after* you get it
            if (!token) {
                throw new Error("Failed to retrieve a valid authentication token.");
            }
            ...
            // Call the corrected API function with options AND the token
            const result = await callGenerationAPI(options, token);

            toast.dismiss(loadingToast);
            toast.success(
                `✅ Generated ${result.data.generated} contacts successfully!`,
                { duration: 4000 }
            );

            if (onContactsGenerated) onContactsGenerated(result);
     ...
     const callGenerationAPI = async (options, token) => {
        if (!token) {
            throw new Error("Authentication token was not provided to API call.");
        }
        ...
        const response = await fetch('/api/admin/generate-contacts', {
            ...
            headers: {
                ...
                // Use the 'token' variable that was passed into this function
    ...
    const handleCustomGenerate = async () => {
        ...
        setIsGenerating(true);
        const loadingToast = toast.loading('Generating custom contacts...');
        ...
        try {
            ...
            // Pass both customOptions and the token
            const result = await callGenerationAPI(customOptions, token);

            toast.dismiss(loadingToast);
            toast.success(
                `✅ Generated ${result.data.generated} custom contacts!`,
                { duration: 4000 }
            );
            ...
            <div className="p-6">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                            ...
                            <div className="text-center">
                                <div className="text-2xl font-bold text-orange-600">
                                    {generationInfo.currentStats.byStatus.new}
                                </div>
                                <div className="text-xs text-gray-600 font-medium">New Status</div>
                            </div>
                        </div>
                    </div>
                )}
                ...
                {!showAdvanced ? (
                    ...
                    <div className="space-y-6">
                        ...
                        <div>
                            ...
                            >
                                {isGenerating ? '⏳ Generating...' : '🚀 Run Selected Scenario'}
                            </button>
                        </div>
                        ...
                        <div className="border-t pt-6">
                            ...
                            >
                                {isGenerating ? '⏳ Generating Custom Contacts...' : '🎯 Generate Custom Contacts'}
                            </button>
                        </div>
                    </div>
                )}
                ...
                <div className="mt-6 p-4 bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-xl">
                    ...
                    <ul className="text-xs text-amber-800 space-y-1">
                        <li>• Start with "Auto-Grouping Test" to see contact grouping features in action</li>
                        <li>• Use "Event Networking" to test event-based filtering and maps</li>
                        <li>• Generate 50-100 contacts for optimal demo experience</li>
                        <li>• Mix event and random contacts to test different data scenarios</li>
                        <li>• Check the map view after generating contacts with location data</li>
                    </ul>
                </div>
            </div>
        </div>
